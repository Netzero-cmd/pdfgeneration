...
const formatCurrency = (amount) => {
    // Check if amount is a number or numeric string; if so, format to two decimal places
 ...
 * for the Magma Commercial Vehicle policy quote, ready for PDF generation services.
 *
 * @param {Object} data - The complete quote data object (see magma_quote_data_structure.js for schema).
 * @returns {string} - The complete, self-contained HTML markup string for the quote.
 */
function generateMagmaQuoteHTML(data) {
    const { quote, vehicle, idv, premium } = data;
...
<head>
    ...
    <title>Magma Commercial Vehicle Quote</title>
    ${internalCSS}
</head>
<body>
